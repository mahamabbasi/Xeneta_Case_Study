{{ config(
    materialized='table',
    pre_hook= "{{ generate_count() }}",
    post_hook= "CREATE OR REPLACE table post_count as (SELECT day, Equipment_ID, Origin_PID,Destination_PID,dq_ok, count(*) as count from {{this}} WHERE dq_ok ='True' group by 1,2,3,4,5 order by count desc)"
)}}
